...
class CustomTimedRotatingFileHandler(TimedRotatingFileHandler):
    ...
    def rotation_filename(self, default_name):
        return os.path.join("logs", str(datetime.date.today() - datetime.timedelta(days=1)) + ".log")
...
log_filename = os.path.join("logs", "temp.log")
file_handler = CustomTimedRotatingFileHandler(log_filename)
file_handler.suffix = "%Y-%m-%d.log"
file_handler.setLevel(logging.INFO)

# Create formatter and add to file handler
formatter = logging.Formatter(fmt="[{asctime}] - [{levelname}] - user{id} - {funcName} - {message} - {status.value}",
                                  datefmt="%Y-%m-%d", style='{')
